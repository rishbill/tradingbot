...
buy_parameter_max_portfolio_percent_per_trade = 0.15
    # Max % of portfolio to spend on a single trade.
...
sell_condition_take_profit_percent = True
    # Take Profit: Good for locking in profit when a "feel-good" profit level is reached i.e. "I at least want to beat SPY". Can be dynamically calculated by % of current price or profit.

sell_condition_fibonacci_atr_stop_price = True
...
sell_condition_trailing_stop_percent = True
...
sell_condition_stop_loss_percent = True
    # Stop Loss %: Sell if position loss hits this fixed %. Good in case ATR or Trailing Stop Loss Prices fail or are to high, to avoid losing too much on the position.

sell_condition_atr_stop_loss_price = True
...
sell_parameter_trailing_stop_percent = 0.10 
    # Sell share when it drops x% from the stock's highest price since purchase. 

sell_parameter_stop_fibonacci_retracement_levels = [0.236, 0.382, 0.618] 
    # key Fibonacci retracement levels to determine potential stop prices. These levels are based on the Fibonacci sequence, a well-known series in mathematics, and are commonly used in technical analysis as indicators of potential reversal points in an asset's price. Traders often observe these specific ratios (23.6%, 38.2%, 61.8%) for signs of price support or resistance
...
sell_parameter_atr_stop_loss_price_multiplier = 2 
...
sell_parameter_rsi_max_threshold = 30
    # The maximum threshold for RSI to be considered weak. Below 30 can suggest downward momentum.
...
stock_filter_parameter_min_price = 3.00 # Require stock to be at least this price to limit risk.
stock_filter_parameter_max_stock_price_portfolio_percent = 0.10 # Limit max stock price to X% of portfolio size for affordability and diversification.
stock_filter_parameter_min_pe_ratio = 0 # Require stock to have positive Profit to Earnings Ratio.
stock_filter_parameter_max_pe_ratio = 20 # Require stock to not be overvalued / overbought.
stock_filter_parameter_min_revenue_growth_percent = 0 # Require stock to have positive Revenue Growth for past year.
