...
# Functions for getting sector info
    ...
    # Gets the total portfolio value for a provided sector 
        try:    
            for symbol in self.Portfolio.Keys: # Iterate over all stocks in portfolio
                if self.Portfolio[symbol].Invested and self.Securities[symbol].Fundamentals.AssetClassification.MorningstarSectorCode == sector: # If the portfolio is invested in this stock, and the sector for this stock matches the sector being checked
    ...
    # Returns a list of stock counts for each sector in the current portfolio.
        try:
            for symbol in self.Portfolio.Keys: # Iterate over all stocks in portfolio
                if self.Portfolio[symbol].Invested: # If the portfolio is invested in this stock
                    v.symbol_sector[symbol] = self.Securities[symbol].Fundamentals.AssetClassification.MorningstarSectorCode # Get the sector for this stock
