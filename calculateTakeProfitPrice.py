...
def calculateTakeProfitPrice(self, symbol, data):
    try:
        if symbol in data and data[symbol] is not None and hasattr(data[symbol], 'Price'): # Confirm this is a valid data point
            ...
            v.take_profit_atr_price[symbol] = v.current_price[symbol] - (v.atr[symbol] * c.sell_parameter_take_profit_price_atr_multiplier) if c.sell_condition_take_profit_atr_price else 0            
            ...
            v.take_profit_fib_atr_price[symbol] = min(v.take_profit_fibonacci_prices[symbol]) + v.atr[symbol] if c.sell_condition_take_profit_fibonacci_atr_price else 0
            ...
            v.take_profit_trailing_price[symbol] = v.current_price[symbol] * (1 + c.sell_parameter_take_profit_trailing_percent) if c.sell_condition_take_profit_trailing_percent else 0
            ...
            v.take_profit_percent_price[symbol] = v.current_price[symbol] * (1 + c.sell_parameter_take_profit_percent) if c.sell_condition_take_profit_percent else 0
            ...
            v.take_profit_max_price[symbol] = max(v.take_profit_percent_price[symbol], v.take_profit_fib_atr_price[symbol], v.take_profit_trailing_price[symbol])
