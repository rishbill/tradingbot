...
def calculateStopLossPrice(self, symbol, data):
    try:
        if symbol in data and data[symbol] is not None and hasattr(data[symbol], 'Price'): # Confirm this is a valid data point
            ...
            v.stop_loss_atr_price[symbol] = v.current_price[symbol] - (v.atr[symbol] * c.sell_parameter_stop_loss_price_atr_multiplier) if c.sell_condition_stop_loss_atr_price else 0
            ...
            v.stop_loss_fib_atr_price[symbol] = min(v.stop_loss_fibonacci_prices[symbol]) + v.atr[symbol] if c.sell_condition_stop_loss_fibonacci_atr_price else 0
            ...
            v.stop_loss_max_price[symbol] = max(v.stop_loss_percent_price[symbol], v.stop_loss_atr_price[symbol], v.stop_loss_trailing_price[symbol])
